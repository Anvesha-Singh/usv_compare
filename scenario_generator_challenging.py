...
# Generate challenging scenarios that expose algorithm weaknesses
...
class ChallengeScenarioBuilder:
    ...
    def _channel_scenario(difficulty: str) -> tuple:
        ...
        if difficulty == 'easy':
            ...
            # Very narrow channel (1.2m) + obstacles inside
            for x in np.linspace(1, 11, 20):
    ...
    def _maze_scenario(difficulty: str) -> tuple:
        ...
        if difficulty == 'easy':
            ...
            # Very complex maze (requires backtracking)
    ...
    def _crowded_scenario(difficulty: str) -> tuple:
        ...
        # Generate crowded field with higher density near start-goal line
        for i in range(num_obs):
            x = random.uniform(0.5, 11.5)
            # More obstacles near centerline
            if random.random() < 0.7:
                y = random.gauss(0, 1.0)  # Gaussian around center
            else:
